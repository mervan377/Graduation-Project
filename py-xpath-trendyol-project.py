...
def job():
    cnx = mysql.connector.connect(user='userbane', password='pass', host='hostaddress', database='database')
    cursor = cnx.cursor()
    ...
    for i in urls:
        ...
        for no, url in enumerate(product_urls, 1):
            ...
            time.sleep(0.50)
            try: #TODO USE IFF
                product_price = str(product_dom.xpath('//span[@class="prc-dsc"]/text()')[0].split(' TL')[0].replace(',', '.'))
            except Exception as e:
                try:
                    product_price = str(product_dom.xpath('//span[@class="prc-slg"]/text()')[0].split(' TL')[0].replace(',', '.'))
                except:
                    product_price = str(product_dom.xpath('//span[@class="prc-org"]/text()')[0].split(' TL')[0].replace(',', '.'))
            try:
                product_price_2 = str(product_dom.xpath('//span[@class="prc-dsc"]/text()')[0].split(' TL')[0].replace(',', '.'))
            except Exception:
                try:
                    product_price_2 = str(product_dom.xpath('//span[@class="prc-slg prc-slg-w-dsc"]/text()')[0].split(' TL')[0].replace(',', '.'))
                except Exception:
                    product_price_2 = str(product_dom.xpath('//span[@class="prc-slg"]/text()')[0].split(' TL')[0].replace(',', '.'))
